...
async function main() {
  ...
  const endpoints = [
    'ws://localhost:6666',
    'ws://localhost:6666/playwright',
    'ws://localhost:6666/chrome',
  ];
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      browser = await chromium.connect(endpoint, { timeout: 10000 });
  ...
  if (!browser) {
    console.error('❌ 无法连接 Browserless，请检查端点配置');
    return;
  }
  ...
  for (let i = 0; i < Math.min(3, 博主元素.length); i++) {
    try {
      ...
      const粉丝数 = await card.$eval('.like-count', el => el.textContent?.trim()) || '未知';
  ...
  console.log('- [x] 连接成功:', usedEndpoint);
  console.log('');
