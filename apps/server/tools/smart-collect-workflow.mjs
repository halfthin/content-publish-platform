...
async function runSmartAnalyzer(keyword = '穿搭') {
  ...
  try {
    ...
    const { stdout, stderr } = await execAsync(
      ...
      {
        cwd: '/home/halfthin/dev/sop/content-publish-platform/apps/server',
        timeout: 120000,
      }
    );
    ...
    // 读取分析结果
    const resultPath = '/home/halfthin/dev/sop/content-publish-platform/.workspace/tests/smart-analyzer-result.json';
...
async function updateSelectorConfig(analysisResult) {
  ...
  try {
    // 读取现有配置
    const configPath = '/home/halfthin/dev/sop/content-publish-platform/selector.conf.json';
...
async function collectDataWithConfig(config, options = {}) {
  ...
  try {
    ...
    const outputFile = `/home/halfthin/dev/sop/content-publish-platform/.workspace/tests/collected-data-${new Date().toISOString().slice(0, 19)}.json`;
